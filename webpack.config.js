...
//entries.vendors = ["react","react-dom"];
	...
	module: {
		loaders: [
			...
			{ test: /\.(jpe?g|png|gif|svg)$/i, loaders: [
					'url?limit=100&name=assets/img/[name].[ext]'
			]},	
		...
		//如果当前页面不需要载入antd,可以考虑将下面代码打开，antd依赖项太多，影响打包性能。
		//noParse: [/antd/]
	},
	plugins: [
...
if(args.e != "development"){
	defaults.module.loaders = [
			...
			{ test: /\.(jpe?g|png|gif|svg)$/i, loaders: [
					'url?limit=100&name=img/[name].[ext]'
			]},	
			{test: /\.jsx?$/, exclude: /(node_modules|bower_components)/,  loader: 'babel',query: { presets: ['react', 'es2015']} },	
			{ test: /\.(woff|eot|ttf)$/i, loader: 'url?name=fonts/[name].[ext]' },
			{ test: /\.(html|jsp|vm)$/, loader: 'html?minimize=false' },
